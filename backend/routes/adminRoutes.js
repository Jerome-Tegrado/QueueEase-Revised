...
router.post('/users', (req, res) => {
  ...
  const insertUserQuery = `
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `;
  db.run(insertUserQuery, [first_name, last_name, address, zip_code, contact_number, email, password, role || 'user'], function (err) {
    if (err) {
...
router.put('/queue/:queueNumber/:action', (req, res) => {
  ...
  db.run(updateQueueQuery, [status, queueNumber], function (err) {
    ...
    db.get(fetchTransactionQuery, [queueNumber], (err, transaction) => {
      ...
      const message =
        status === 'in-progress' ? `Your transaction #${queueNumber} is now in progress.` :
        status === 'completed' ? `Transaction #${queueNumber} has been completed.` :
        `Transaction #${queueNumber} has been canceled.`;

      const insertNotificationQuery = `
        INSERT INTO notifications (user_id, message) VALUES (?, ?)
      `;
      db.run(insertNotificationQuery, [transaction.user_id, message], (notifErr) => {
        if (notifErr) console.error('Failed to notify user:', notifErr.message);
      });
